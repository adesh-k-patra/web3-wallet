/*
  ...
  - Changed the type of `private_key` on the `Account` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  - Changed the type of `seed_phrase` on the `SecretSeed` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
-- AlterTable
ALTER TABLE "SecretSeed" DROP COLUMN "seed_phrase",
